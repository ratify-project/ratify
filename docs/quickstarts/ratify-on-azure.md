...
run `az aks show -g "${GROUP_NAME}" -n "${AKS_NAME}" --query addonProfiles.azurepolicy` to verify if the AKS cluster enables azure policy addon, for more [details](https://learn.microsoft.com/en-us/azure/aks/use-azure-policy)
