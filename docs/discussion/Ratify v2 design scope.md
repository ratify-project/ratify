...
1. We need to refactor the whole plugin framework to make it more lightweight and concurrent-safe. Specifically, we have 2 options:
   1. Use a plugin framework like [HashiCorp go-plugin](https://github.com/hashicorp/go-plugin) to make Ratify main process communicate with external verifiers through GRPC. And we can have Ratify main process to manage the cache and OCI store while external verifiers will query/set cache through GRPC call.
   2. We keep external plugins run as a command in subprocess and use IPC mechanism to communicate with Ratify main process. We can use pipe to share cache and OCI store among external verifiers and Ratify main process. This could achieve the same goal as the first option but with less overhead of network communication.
2. We may have all built-in plugins as external plugins. This will make it easier to maintain and extend the built-in plugins. We can also have a separate repo for built-in plugins.
...
2. It's not straightforward to convert configurations between CLI and service. Users may need extra effort to learn the difference between CLI and service configurations.
3. Oras store only supports one auth provider. One of the reasons is that the Oras store configuration does not support multiple auth providers. Related issue: [issue-974](https://github.com/ratify-project/ratify/issues/974).

**Proposed Solution**:
1. Refactor both CLI and service configurations to make them consistent and convertable. We can have a common configuration format that can be used by both CLI and service. We hope to make the new configuration format extensible to standalone service and GRPC service.
...
**Current Limitation**:
1. Currently Ratify only supports CLI and Gatekeeper add-on user scenarios. In the future, we may also supports GRPC service and standalone service user scenarios. But the current codebase have everything in the same repo which makes it hard to maintain and extend.
...
3. Extract out the Ratify core and different user scenarios to different packages. As a first step, we could just create different folders for different scenarios. And in the future, we can consider moving them to different repos.
