...
func TestGetCertFromSecretBundle(t *testing.T) {
	...
	// invalid PkcS
	testData = getSecretBundleInvalidPkcs()
	certs, status, err = getCertsFromSecretBundle(testData, "certName")
	assert.Equal(t, len(certs), 0)
	assert.Equal(t, len(status), 0)
