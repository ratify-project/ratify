...
func (f *ristrettoFactory) Create(ctx context.Context, cacheEndpoint string, cacheSize int) (cache.CacheProvider, error) {
	var err error
...
func (r *ristrettoCache) Clear(ctx context.Context) bool {
	r.memoryCache.Clear()
	// Note: ristretto does not return a bool for clear.
	// Clear ops are eventually consistent and we don't want to block on them.
	return true
}
