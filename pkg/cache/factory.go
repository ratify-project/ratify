...
type CacheFactory interface {
	Create(ctx context.Context, cacheEndpoint string, cacheSize int) (CacheProvider, error)
}
...
func NewCacheProvider(ctx context.Context, name string, cacheEndpoint string, cacheSize int) (CacheProvider, error) {
	...
	var err error
	memoryCache, err = factory.Create(ctx, cacheEndpoint, cacheSize)
	if err != nil {
		return nil, err
	}
