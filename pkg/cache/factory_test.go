...
func (t TestCache) Get(ctx context.Context, key string) (string, bool) {
	return "", false
}

func (t TestCache) Set(ctx context.Context, key string, value interface{}) bool {
	return false
}

func (t TestCache) SetWithTTL(ctx context.Context, key string, value interface{}, ttl time.Duration) bool {
	return false
}

func (t TestCache) Delete(ctx context.Context, key string) bool {
	return false
}

func (t TestCache) Clear(ctx context.Context) bool {
	return false
}

func (t TestCacheFactory) Create(ctx context.Context, cacheEndpoint string, cacheSize int) (CacheProvider, error) {
	if t.shouldError {
...
func TestNewCacheProvider_Expected(t *testing.T) {
	...
	_, err := NewCacheProvider(context.Background(), "test-newcacheprovider-expected", "localhost:6379", DefaultCacheSize)
	if err != nil {
...
func TestNewCacheProvider_NotFound(t *testing.T) {
	...
	_, err := NewCacheProvider(context.Background(), "notfound", "localhost:6379", DefaultCacheSize)
	if err == nil {
...
func TestNewCacheProvider_Error(t *testing.T) {
	...
	_, err := NewCacheProvider(context.Background(), "test-newcacheprovider-error", "localhost:6379", DefaultCacheSize)
	if err == nil {
		t.Errorf("Expected error, got nil")
	}
}
...
func TestGetCacheProvider_Expected(t *testing.T) {
	...
	_, err := NewCacheProvider(context.Background(), "test-getcacheprovider-expected", "localhost:6379", DefaultCacheSize)
	if err != nil {
		t.Errorf("Expected no error, got %s", err)
	}
