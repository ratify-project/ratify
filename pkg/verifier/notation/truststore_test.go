...
func TestGetCertificates_NamedStore(t *testing.T) {
	...
	ctx = ctxUtils.SetContextWithNamespace(ctx, "default")

	// only the certificate in the specified namedStore should be returned
	result, _ := store.getCertificatesInternal(ctx, "store1", certificatesMap)
	expectedLen := 1
	...
	// Fetching certificates in different namespace.
	ctx = ctxUtils.SetContextWithNamespace(ctx, "projectb")
	result, _ = store.getCertificatesInternal(ctx, "store1", certificatesMap)
...
func TestGetCertificates(t *testing.T) {
	...
	trustStore.certStores = map[string][]string{
		"store1": {"projecta/store1"},
	}
	if _, err := trustStore.GetCertificates(ctx, "", "store1"); err == nil {
		t.Fatalf("expected to return error but got nil")
	}

	controllers.CertificatesMap = cs.NewActiveCertStores()
	controllers.CertificatesMap.AddStore("default", "default/store1", []*x509.Certificate{getCert(certStr)})
	trustStore.certStores = map[string][]string{
		"store1": {"default/store1"},
	}
	ctx = ctxUtils.SetContextWithNamespace(ctx, "default")
