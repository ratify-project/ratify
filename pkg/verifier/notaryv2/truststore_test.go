...
func TestGetCertificates_EmptyCertMap(t *testing.T) {
	...
	if _, err := store.getCertificatesInternal(context.Background(), truststore.TypeCA, "store1", certificatesMap); err == nil {
...
func TestGetCertificates_certPath(t *testing.T) {
	...
	certs, err := trustStore.getCertificatesInternal(context.Background(), truststore.TypeCA, "", nil)
	if err != nil {
...
func TestFilterValidCerts(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "invalid cert",
			certs:     []*x509.Certificate{getCert(leftCertStr)},
			expect:    nil,
			expectErr: true,
		},
	}
